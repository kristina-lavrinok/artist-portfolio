...
export default function ArtworkCarousel({ imageUrls, title, status }: ArtworkCarouselProps) {
  ...
  return (
    <div className="relative group">
      <div className="relative overflow-hidden">
        <img
          src={`${import.meta.env.BASE_URL}${imageUrls[currentIndex].replace(/^\//, '')}`}
          alt={`${title} - Image ${currentIndex + 1}`}
          className="w-full h-auto transition-opacity duration-500 hover:opacity-90"
        />
        
        {/* Status Label */}
        <div className={`absolute bottom-4 left-4 px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
          status === 'sold' ? 'bg-black/50 text-white' : 'bg-black/50 text-white'
        }`}>
